#!/usr/bin/env python3
"""
Virgilio spec-validator for OpenAI Codex.

Purpose:
- Enforce the specification-first rule for Codex.
- Block code-writing actions when SPEC.md is missing from the project root.

Notes:
- Codex PreToolUse hooks receive a JSON object on stdin.
- File edits normally arrive as apply_patch; shell commands arrive as Bash.
- This hook is intentionally conservative: it only blocks when it can detect that a
  command or patch is trying to create/modify source-code files.
"""
...
def path_mentions(text: str) -> list[tuple[str, str]]:
    # Accept paths in plain commands and patch headers.
...
def extract_apply_patch_targets(command: str) -> list[str]:
    """Return target paths declared by an apply_patch command.

    Codex's apply_patch format prefixes each file operation with a header
    such as `*** Add File: path/to/foo.py`. Pulling these targets out lets
    us judge a patch by the files it touches, not by every path that
    happens to appear in the surrounding text.
    """
...
def is_code_write_attempt(payload: dict[str, Any]) -> bool:
    ...
    # apply_patch carries a structured patch in tool_input.command. Judge
    # the patch by the files it touches, not by the surrounding text.
    if tool_name == "apply_patch":
        ...
        if targets:
            if all(path_is_allowed_without_spec(p) for p in targets):
                return False
            return targets_force_block(targets)
        # No structured targets found — fall back to content scan.
        if mentions_allowed_infrastructure_only(command):
            return False
        return mentions_code_path(command)
    ...
    if tool_name == "Bash":
        ...
        if setup_attempt:
            return True
        if mentions_allowed_doc_only(command) or mentions_allowed_infrastructure_only(command):
            return False
        return mentions_code_path(command)

    # File-edit tools (Write, Edit, MultiEdit, MCP filesystem tools) expose
    # the target file path explicitly. Trust the path, not the file body.
    ...
    # Final fallback: scan all strings.
    text = "\n".join(iter_strings(tool_input))
